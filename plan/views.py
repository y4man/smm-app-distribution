...
# Optimized code
def merge_dict_fields(instance, data, fields):
    for field in fields:
        if field in data:
            current_data = getattr(instance, field, {}) or {}
            current_data.update(data.get(field, {}))
...
class PlanView(generics.ListCreateAPIView, generics.RetrieveUpdateDestroyAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        merge_fields = ['pricing_attributes', 'standard_attributes', 'advanced_attributes', 'pricing_platforms']
        data = merge_dict_fields(plan, data, merge_fields)
...
class UnassignedAccountManagerSearchView(APIView):
    ...
    def post(self, request):
        filters = {
            'role': request.data.get('role', 'account_manager'),
            'agency_name__icontains': request.data.get('agency_name'),
            'first_name__icontains': request.data.get('first_name'),
            'last_name__icontains': request.data.get('last_name'),
        }

        # Remove None values
        filters = {k: v for k, v in filters.items() if v is not None}

        account_managers = CustomUser.objects.filter(
            Q(**filters) & Q(assigned_plans__isnull=True)
        )
...
class AssignedAccountManagerSearchView(APIView):
    ...
    def get(self, request):
        ...
        account_managers = plan.account_managers.all()
        serializer = UserSerializer(account_managers, many=True)
        return Response(serializer.data)
...
class AssignedPlansForAccountManagerView(APIView):
    ...
    def get(self, request, client_id):
        ...
        plans = account_manager.assigned_plans.all()
...
class RemoveAccountManagerFromPlanView(APIView):
    ...
    def post(self, request):
        ...
        if plan.account_managers.filter(id=account_manager.id).exists():
            plan.account_managers.remove(account_manager)
            return Response({
                "message": f"Account Manager '{account_manager.username}' removed from Plan '{plan.plan_name}'."
            })
        return Response({
            "error": f"Account Manager '{account_manager.username}' is not assigned to Plan '{plan.plan_name}'."
        }, status=status.HTTP_400_BAD_REQUEST)
