...
class BaseTaskSerializer(serializers.ModelSerializer):
    # Base serializer with common methods for task serializers
    assigned_to_name = serializers.SerializerMethodField()
    client_name = serializers.CharField(source='client.business_name', read_only=True)

    class Meta:
        model = Task  # Base model for Task-related serializers
        fields = []  # Will be overridden in child classes

    def get_assigned_to_name(self, obj):
        # Get full name of assigned user
        if not obj.assigned_to:
            return None
        return f"{obj.assigned_to.first_name or ''} {obj.assigned_to.last_name or ''}".strip()
...
class CustomTaskSerializer(serializers.ModelSerializer):
    ...
    def get_task_file(self, obj):
        # Generate full URL for task file if it exists
        if not obj.custom_task_file:
            return None
        return (
            f"{settings.SUPABASE_URL}/storage/v1/object/public/"
            f"{settings.SUPABASE_BUCKET}/{obj.custom_task_file}"
        )
...
class MyTaskSerializer(BaseTaskSerializer):
    # Serializer for tasks assigned to the current user
    ...
    class Meta(BaseTaskSerializer.Meta):
        fields = BaseTaskSerializer.Meta.fields + [
            'id', 'client', 'created_at', 'task_type', 'client_business_name'
        ]


class TaskSerializer(BaseTaskSerializer):
    # Serializer for general task operations
    
    class Meta(BaseTaskSerializer.Meta):
        fields = BaseTaskSerializer.Meta.fields + [
            'id', 'task_type', 'client', 'client_name',
